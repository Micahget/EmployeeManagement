    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (request.getParameter("register") != null)//request comming from index.jsp page, where register is button name.
            {
                ...
                System.out.println(id + " " + name + " " + email + " " + password + " " + department + " " + designation + " " + todayDate);
                /* JDBC Connection Code  */
                System.out.println("Debbuging 1");
                con = DB.getConnection();
                st = con.createStatement();
                String sql = "insert into employee(id , name,email,password,department,designation,added_date) values('" + id + "','" + name + "','" + email + "','" + password + "','" + department + "','" + designation + "','" + todayDate + "')";
                status = st.executeUpdate(sql);
                System.out.println("Debbuging");

                /* send email to employees.jsp file */
                if (status > 0) {
